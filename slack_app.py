...
STORE_FILE = os.environ.get("BOOKING_STORE_FILE", "booking_thread_store.json")
...
def post_msg(channel, blocks, text, ts=None):
    ...
    res = slack("chat.postMessage", payload)
    return res.get("ts") if res.get("ok") else None
...
def plate_from_car(car):
    car = str(car or "")
    if "[" in car and "]" in car:
        return car.rsplit("[", 1)[1].split("]", 1)[0].strip()
    return ""


def update_delivery_store(booking, delivery_ts):
    store = load_store()
    bookings = store.setdefault("bookings", {})
    contract = str(booking.get("id") or "").strip()
    plate = plate_from_car(booking.get("car"))

    entry = bookings.get(contract) if contract else None
    if entry is None:
        for key, candidate in bookings.items():
            if not isinstance(candidate, dict):
                continue
            if contract and contract in key:
                entry = candidate
                break
            if plate and str(candidate.get("plate") or "").strip() == plate:
                entry = candidate
                break

    if entry is None:
        entry = {}
        bookings[contract or f"delivery-{delivery_ts}"] = entry
    ...
    save_store(store)
    print(f"delivery store updated: {contract or plate} -> {delivery_ts}")
...
def verify(body, ts, sig):
    try:
        if abs(time.time() - int(ts)) > 300:
            print("verify FAILED: timestamp too old")
            return False
...
def delivery_modal(b, trigger, ch, ts):
    open_modal(trigger, {
        ...
        "blocks": [
            ...
            {"type": "divider"},
            {"type": "input", "block_id": "driver_name", "label": {"type": "plain_text", "text": "Driver Name"}, "element": {"type": "plain_text_input", "action_id": "value", "placeholder": {"type": "plain_text", "text": "e.g. Ahmed"}}},
            {"type": "input", "block_id": "delivery_time", "label": {"type": "plain_text", "text": "Delivery Time (GCC 24h)"}, "element": {"type": "plain_text_input", "action_id": "value", "initial_value": gcc_now(), "placeholder": {"type": "plain_text", "text": "e.g. 14:30"}}},
            {"type": "input", "block_id": "out_km", "label": {"type": "plain_text", "text": "Out KM"}, "element": {"type": "plain_text_input", "action_id": "value", "placeholder": {"type": "plain_text", "text": "e.g. 12500"}}},
            {"type": "input", "block_id": "fuel_level", "label": {"type": "plain_text", "text": "Fuel Level"}, "element": {"type": "static_select", "action_id": "value", "placeholder": {"type": "plain_text", "text": "Select fuel level"}, "options": FUEL_OPTIONS}},
            {"type": "input", "block_id": "photos_uploaded", "label": {"type": "plain_text", "text": "Photos Uploaded"}, "element": {"type": "static_select", "action_id": "value", "placeholder": {"type": "plain_text", "text": "Select"}, "options": [{"text": {"type": "plain_text", "text": "Yes"}, "value": "Yes"}, {"text": {"type": "plain_text", "text": "No"}, "value": "No"}]}},
            {"type": "input", "block_id": "remarks", "optional": True, "label": {"type": "plain_text", "text": "Remarks"}, "element": {"type": "plain_text_input", "action_id": "value", "multiline": True, "placeholder": {"type": "plain_text", "text": "Optional"}}},
        ],
    })
...
def pickup_modal(b, trigger, ch, ts):
    ...
    open_modal(trigger, {
        ...
        "blocks": [
            ...
            {"type": "input", "block_id": "fuel_charge", "optional": True, "label": {"type": "plain_text", "text": "Fuel Charge"}, "element": {"type": "plain_text_input", "action_id": "value", "placeholder": {"type": "plain_text", "text": "e.g. AED 0"}}},
            {"type": "input", "block_id": "damage_charges", "optional": True, "label": {"type": "plain_text", "text": "Damage Charges"}, "element": {"type": "plain_text_input", "action_id": "value", "placeholder": {"type": "plain_text", "text": "e.g. AED 0"}}},
            {"type": "input", "block_id": "amount_collected", "label": {"type": "plain_text", "text": "Amount Collected"}, "element": {"type": "plain_text_input", "action_id": "value", "placeholder": {"type": "plain_text", "text": "e.g. AED 2,143"}}},
            {"type": "input", "block_id": "payment_mode", "label": {"type": "plain_text", "text": "Payment Mode"}, "element": {"type": "static_select", "action_id": "value", "placeholder": {"type": "plain_text", "text": "Select"}, "options": [{"text": {"type": "plain_text", "text": "Cash"}, "value": "Cash"}, {"text": {"type": "plain_text", "text": "Card"}, "value": "Card"}, {"text": {"type": "plain_text", "text": "Bank Transfer"}, "value": "Bank Transfer"}, {"text": {"type": "plain_text", "text": "Crypto"}, "value": "Crypto"}]}},
            {"type": "input", "block_id": "remarks", "optional": True, "label": {"type": "plain_text", "text": "Remarks"}, "element": {"type": "plain_text_input", "action_id": "value", "multiline": True, "placeholder": {"type": "plain_text", "text": "Optional"}}},
        ],
    })
...
def handle_km_update(payload):
    try:
        ...
        try:
            ...
            km_text = "*KM Driven:* - (auto-calculated)"
